...
LoadBalancer::HandleRead(Ptr<Socket> socket)
{
    ...
    while ((packet = socket->RecvFrom(from)))
    {
        ...
        //printf("LB : %d %d\n", requestType.GetSeq() / 10, requestType.GetSeq() % 10);
        SeqTsHeader clientHeader;
        packet->RemoveHeader(clientHeader);
        uint16_t fromPort = clientHeader.GetSeq();
        packet->RemoveHeader(clientHeader);
        uint32_t requestType = clientHeader.GetSeq();
        packet->RemoveHeader(clientHeader);
        uint32_t seq = clientHeader.GetSeq();

        packet->RemoveAllPacketTags();//없애보기
        packet->RemoveAllByteTags();

        clientHeader.SetSeq(seq);
        packet->AddHeader(clientHeader);
        clientHeader.SetSeq(requestType);
        packet->AddHeader(clientHeader);
